...
function calculateSalmonTokenScoring() {
    ...
    function getRuns() {
        let groups = getAnimalGroups("salmon");
        let runs = [];
        outer:
        for (let group of groups) {
            for (let tile in group) {
                let neighbours = searchNeighbourTilesForWildlife(tile, "salmon");
                // if there is a salmon with more than 2 salmons next to it, this group is not a valid run
    ...
    let score = 0;
    switch (currentSets["salmon"]) {
        // A: scores for each salmon run
        case "a": {
            let runs = getRuns();
            let scoring = {
                1: 2,
        ...
        // B: scores for each salmon run. only small runs
        case "b": {
            let runs = getRuns();
            let scoring = {
                1: 2,
    ...
    tokenScoring.salmon.totalScore += score;
}
